exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!product || !id) {
      throw new Error('Product and ID parameters are required');
    }

    const response = await fetch(`${DM_API_BASE}/TrackingDevice/GetBatteryPercentageAndDeviceCounters?product=${product}&id=${id}`, {
      method: 'GET',
      headers: {
